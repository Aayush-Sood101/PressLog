...
export const WebcamPixelGrid = ({
  ...
  const render = useCallback(() => {
    ...
    if (!video || !processingCanvas || !displayCanvas || video.readyState < 2) {
      animationRef.current = requestAnimationFrame(renderFnRef.current);
      return;
    }
  ...
  // Store render function in ref to avoid circular dependency
  ...
  return (
    <div className={cn("relative h-full w-full", className)}>
      ...
      {error && showErrorPopup && (
        <div
          className="animate-in fade-in slide-in-from-top-2 fixed top-4 right-4 z-50 duration-300">
          <div
            className="relative flex max-w-sm items-start gap-3 rounded-lg border border-white/10 bg-black/80 p-4 shadow-2xl backdrop-blur-xl">
            {/* Close button */}
            <button
